...
def mpa_cond1(z, E): #, PPcond_rate
   ...
   if(np.real(E) < 0.): E = -E
   if(np.imag(E) > -epsilon): E = complex(np.real(E), -epsilon)
...
def pole_is_out(i, wmax, thr, E): # we need to modify E inside the function
    ...
    if(np.real(E[i]) > wmax): is_out = True

    j=0
    while(j<i and not is_out):
      if(abs(np.real(E[i])-np.real(E[j]))<thr):
        is_out = True
        if(abs(np.real(E[j]))>max(abs(np.imag(E[j])), abs(np.imag(E[i])))):
          E[j] = np.mean([np.real(E[j]), np.real(E[i])]) -1j*max(abs(np.imag(E[j])), abs(np.imag(E[i])))
        else:
          E[j] = np.mean([np.real(E[j]), np.real(E[i])]) -1j*min(abs(np.imag(E[j])), abs(np.imag(E[i])))
      j=j+1
...
def mpa_cond(npols, z, E):
    ...
    wmax = np.max(wr)*1.5 #DALV: we were using the square before, now we use 1.5* the extreme of the interval
    Eaux = np.emath.sqrt(E)

    i=0
    while(i<npr):
      Eaux[i] = max(abs(np.real(Eaux[i])),abs(np.imag(Eaux[i]))) -1j*min(abs(np.real(Eaux[i])),abs(np.imag(Eaux[i])))
...
def mpa_R_1p_fit(npols, npr, w, x, E):
    ...
    A = np.zeros((4,2),dtype='complex128')
...
def mpa_R_fit(npols, npr, w, x, E):
    ...
    """
    A = np.zeros((2*npols,npr),dtype='complex128')
...
##endif
    ...
    # Transforming the problem into a 2* larger least square with real numbers:
    A = np.zeros((2*npols*2,npr*2),dtype='complex128')
    b = np.zeros((2*npols*2),dtype='complex128')
    for k in range(2*npols):
      b[2*k]   = np.real(x[k])
      b[2*k+1] = np.imag(x[k])
      for i in range(npr):
    ...
    R = np.zeros(npols,dtype='complex128')
...
def mpa_E_solver_Pade(npols, z, x):
    ...
    #PPcond[:] = True
    b_m1 = b = np.zeros(npols+1, dtype='complex128')
    b_m1[0] = b[0] = 1
    c = np.copy(x)
    ...
    Companion = np.polynomial.polynomial.polycompanion(b[:npols+1]) #/b[npols] it is carried inside
...
##endif
    ...
    #E = np.linalg.eigvals(Companion) #DALV: in general the companion matrix is not a normal
    E = eigvals(Companion)
...
def mpa_RE_solver(npols, w, x):
    ...
    if(npols == 1): #DALV: we could also particularize the solution for 2 and 3 poles
      E, PPcond_rate = mpa_E_1p_solver(w, x)
      R = mpa_R_1p_fit(1, 1, w, x, E) #DALV: if PPcond_rate=0, R = x[1]*(z[1]**2-E**2)/(2*E)
      MPred=0
    else:
...
E = [complex(1,0.1), complex(1.0001,-0.11), complex(2,0.2), complex(18,1)]
