...
for q in [1]:
    ...
    axs1[0,0].plot(energy,eps_r,label='IP d0.1eV q'+str(q),c='orange')
    axs1[1,0].plot(energy,eps_i,c='orange')
    
    energy,eel_i,eel_r = np.genfromtxt(out_ip_q+eel+str(q)+theoIP,autostrip=True,usecols=[0,1,2],unpack=True)
    axs1[0,1].plot(energy,eel_r,label='IP d0.1eV q'+str(q),c='orange')
    axs1[1,1].plot(energy,eel_i,c='orange')
"""
...
for q in [3]:
    ...
    axs5[0,0].plot(energy, ex_r1 ,ls=':',label='FF',c='k')
    axs5[1,0].plot(energy,-ex_i1,ls=':',label='FF',c='k')
    #axs5[0,1].plot(energy, XreMP ,label='MP'+str(npol))
    Xr_w = np.zeros_like(energy)
    Xr_iw = np.zeros_like(energy)

    l = [7,10,8,11,1]
    nl = len(l)
    for p in range(nl):
      Xr_w  = Xr_w + 2*E[l[p]]*R[l[p]]*HatoeV**2/((energy)**2-(E[l[p]]*HatoeV)**2)
      Xr_iw = Xr_iw+ 2*E[l[p]]*R[l[p]]*HatoeV**2/((energy)**2+(E[l[p]]*HatoeV)**2)
      axs5[0,0].plot(energy, np.real(2*E[l[p]]*R[l[p]]*HatoeV**2/((energy+shift*1j)**2-(E[l[p]]*HatoeV)**2)),c=cmap_blue((nl-cfa*p)/nl),label='p'+str(p+1))
      axs5[1,0].plot(energy,-np.imag(2*E[l[p]]*R[l[p]]*HatoeV**2/((energy+shift*1j)**2-(E[l[p]]*HatoeV)**2)),c=cmap_blue((nl-cfa*p)/nl),label='p'+str(p+1))

      axs5[1,1].plot(energy, np.real(Xr_iw),c=cmap_blue((nl-cfa*p)/nl),label='MP'+str(p+1))
      axs5[0,1].plot(energy,-np.imag(Xr_iw),c=cmap_blue((nl-cfa*p)/nl),label='MP'+str(p+1))

    l = [0,2,3,4,5,6,9,12,13,14]
    for p in range(len(l)):
      Xr_w  = Xr_w + 2*E[l[p]]*R[l[p]]*HatoeV**2/((energy)**2-(E[l[p]]*HatoeV)**2)
      Xr_iw = Xr_iw+ 2*E[l[p]]*R[l[p]]*HatoeV**2/((energy)**2+(E[l[p]]*HatoeV)**2)

    axs5[0,0].plot(energy, np.real(Xr_w),label='MP'+str(npol),c='r')
    axs5[1,0].plot(energy,-np.imag(Xr_w),label='MP'+str(npol),c='r')
    axs5[1,1].plot(energy, np.real(Xr_iw),label='MP'+str(npol),c='r')
    axs5[0,1].plot(energy, np.imag(Xr_iw),label='MP'+str(npol),c='r')
...
axs5[1,1].legend(edgecolor='k'),axs5[0,0].legend(edgecolor='k'),axs5[0,1].legend(edgecolor='k'),axs5[1,0].legend(edgecolor='k') #loc=4,borderaxespad=0.3,title=EF_q[ef]
#axs1[0,1].legend(edgecolor='k')
...
axs5[1,0].set_ylabel("$-Im[\epsilon^{-1}]~(a.u.)$")

axs5[1,0].set_xlim(-1,101)
